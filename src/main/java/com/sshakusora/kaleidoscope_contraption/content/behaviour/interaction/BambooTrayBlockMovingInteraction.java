...
public final class BambooTrayBlockMovingInteraction extends BlockEntityDelegatingMovingInteraction {
    @Override
    public boolean handlePlayerInteraction(Player player, InteractionHand hand, BlockPos localPos,
        ...
        boolean changed = held.isEmpty()
                ? tray.onTakeOut(contraptionEntity.level(), player, slot, player.isSecondaryUseActive())
                : tray.onPutItem(contraptionEntity.level(), player, held, slot);
        if (changed) {
    ...
    private boolean removeTray(Player player, BlockPos localPos,
        ...
        removeFromContraption(player, localPos, contraptionEntity, info);
        return true;
    }

    private void removeFromContraption(Player player, BlockPos localPos,
                                        AbstractContraptionEntity contraptionEntity,
                                        StructureTemplate.StructureBlockInfo info) {
