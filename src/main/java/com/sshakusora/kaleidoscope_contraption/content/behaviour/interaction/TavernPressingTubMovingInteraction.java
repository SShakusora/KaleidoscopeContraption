...
public final class TavernPressingTubMovingInteraction extends SyncedMovingInteractionBehaviour {
    ...
    public boolean handlePlayerInteraction(Player player, InteractionHand activeHand, BlockPos localPos,
        ...
        ItemStackHandler items = TavernBrewingContraptionSupport.readItems(nbt, ITEMS, 1, 64);
        FluidTank fluid = TavernBrewingContraptionSupport.readFluid(nbt, IPressingTub.MAX_FLUID_AMOUNT);
        ...
        if (changed) {
            TavernBrewingContraptionSupport.writeItems(nbt, ITEMS, items);
            TavernBrewingContraptionSupport.writeFluid(nbt, fluid);
    ...
    public void handleEntityCollision(Entity entity, BlockPos localPos,
        ...
        PressingTubRecipe recipe = contraptionEntity.level().getRecipeManager()
        ...
        if (!fluid.isEmpty() && !fluid.getFluid().isFluidEqual(produced)) {
            playPressEffect(contraptionEntity, localPos, input, SoundEvents.SLIME_BLOCK_FALL);
            dropInvalidContents(contraptionEntity, localPos, info, nbt, items, fluid);
            return;
        }
