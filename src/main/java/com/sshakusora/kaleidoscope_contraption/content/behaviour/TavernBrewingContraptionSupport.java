...
public final class TavernBrewingContraptionSupport {
    ...
    public static ItemStackHandler readItems(CompoundTag nbt, String key, int slots, int slotLimit) {
        return readItems(nbt, key, slots, slotLimit, RegistryAccess.EMPTY);
    }
    ...
    public static ItemStackHandler readItems(CompoundTag nbt, String key, int slots, int slotLimit,
        ...
        if (nbt.contains(key)) {
            items.deserializeNBT(registries, nbt.getCompound(key));
        }
        return items;
    }

    public static FluidTank readFluid(CompoundTag nbt, int capacity) {
        return readFluid(nbt, capacity, RegistryAccess.EMPTY);
    }

    public static FluidTank readFluid(CompoundTag nbt, int capacity, HolderLookup.Provider registries) {
        FluidTank tank = new FluidTank(capacity);
        if (nbt.contains(FLUID)) {
            tank.readFromNBT(registries, nbt.getCompound(FLUID));
        }
        return tank;
    }

    public static void writeItems(CompoundTag nbt, String key, ItemStackHandler items) {
        writeItems(nbt, key, items, RegistryAccess.EMPTY);
    }

    public static void writeItems(CompoundTag nbt, String key, ItemStackHandler items,
                                  HolderLookup.Provider registries) {
        nbt.put(key, items.serializeNBT(registries));
    }

    public static void writeFluid(CompoundTag nbt, FluidTank fluid) {
        writeFluid(nbt, fluid, RegistryAccess.EMPTY);
    }

    public static void writeFluid(CompoundTag nbt, FluidTank fluid, HolderLookup.Provider registries) {
        nbt.put(FLUID, fluid.writeToNBT(registries, new CompoundTag()));
    }
    ...
    public static ItemStack getBarrelOutput(CompoundTag nbt, HolderLookup.Provider registries) {
        return readItems(nbt, OUTPUT, 1, 64, registries).getStackInSlot(0);
    }
