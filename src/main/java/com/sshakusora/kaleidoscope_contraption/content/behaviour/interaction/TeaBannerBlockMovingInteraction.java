...
public class TeaBannerBlockMovingInteraction extends BlockEntityDelegatingMovingInteraction {
    @Override
    public boolean handlePlayerInteraction(Player player, InteractionHand hand, BlockPos localPos,
        ...
        if (held.getItem() instanceof BannerPatternItem && TeaBannerBlockEntity.isSupportedPattern(held)) {
            if (ItemStack.isSameItemSameComponents(held, banner.getPatternItem())) {
                return true;
            }
            if (contraptionEntity.level().isClientSide) {
                return true;
    ...
    private boolean removeBanner(Player player, BlockPos localPos,
        ...
        if (!player.isCreative()) {
            ItemHandlerHelper.giveItemToPlayer(player, banner.createItemStack(contraptionEntity.level()));
        }
